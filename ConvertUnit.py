...
def main():
    ...
    for widget in range(0, 4):                                                                          
        widgets[widget].append(Label(window, text = label[widget * 2]))
        widgets[widget].append(Entry(window, width = 10, state = 'enabled'))
        widgets[widget].append(Button(window, text = '변환하기', command = lambda arg1 = widget:onClick(arg1)))
        widgets[widget].append(Label(window, text = label[widget * 2 + 1]))
        widgets[widget].append(Entry(window, width = 10, state = 'disabled'))
